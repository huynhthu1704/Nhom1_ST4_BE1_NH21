...
function addReview(id, perpage) {
  ...
  let rating = document.getElementById("rating-star-count").value;
  if (review_name == "" || review_email == "") {
    alert("Please fill in information");
  } else if (rating == -1) {
    alert("Choose your rating star");
  } else {
    let xmlhttp = new XMLHttpRequest();
    xmlhttp.onload = function () {
      let pagination = document.getElementsByClassName("reviews-pagination");
      while (pagination.firstChild) {
        pagination.removeChild(paginate.firstChild);
      }
      pagination.insertAdjacentHTML("afterbegin", this.responseText);
      paginate(id, 1, perpage);
      getReviewInformation(product_id);
    };
    xmlhttp.open(
      "GET",
...
function getReviewInformation(id) {
  let xmlhttp = new XMLHttpRequest();
  xmlhttp.onload = function () {
    let info = this.responseText.split("#");
    // display average rating
    let productRating = document.querySelectorAll("product-rating");
    for (let i = 0; i < productRating.length; i++) {
      while (productRating[i].firstChild) {
        productRating[i].removeChild(productRating[i].firstChild);
      }
    }
    for (let i = 0; i < productRating.length; i++) {
      while (productRating[i].firstChild) {
        productRating[i].insertAdjacentHTML("beforeend", info[2]);
      }
    }

    document.getElementById("average-rating").innerHTML = info[1];
    let totalReview = document.querySelectorAll(".total-review");
    for (let i = 0; i < totalReview.length; i++) {
      totalReview[i].innerHTML = info[0];
    }

    let ratingProgress = document.getElementById("rating-progress-list");
    while (ratingProgress.firstChild) {
      ratingProgress.removeChild(ratingProgress.firstChild);
    }
    ratingProgress.insertAdjacentHTML("beforeend", info[3]);
  };
  xmlhttp.open("GET", "review-information.php?id=" + id);
  xmlhttp.send();
}

function paginate(id, page, perpage) {
  let xmlhttp = new XMLHttpRequest();
  xmlhttp.onload = function () {
    ...
    reviewList.insertAdjacentHTML("afterbegin", this.responseText);
  };
  xmlhttp.open(
    "GET",
    "review-paginate.php?id=" + id + "&page=" + page + "&perpage=" + perpage
  );
  xmlhttp.send();
}

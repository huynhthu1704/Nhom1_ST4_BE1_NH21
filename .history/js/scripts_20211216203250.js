...
function getReviewInformation(id) {
  ...
  xmlhttp.onload = function () {
    ...
    for (let i = 0; i < productRating.length; i++) {
      while (productRating[i].firstChild) {
        productRating[i].insertAdjacentHTML("beforeend", item[2]);
      }
    }

    document.getElementById("average-rating").innerHTML = item[1];

    document.getElementById("total-review").forEach(item, function () {
      item.innerHTML = item[0];
    });
    document.getElementsByClassName("rating-progress").innerHTML = item[3];
  };

...
function getReviewInformation(id) {
    ...
    xmlhttp.onload = function() {
       ...
       for (let i = 0; i < productRating.length; i++) {
        ...
        document.getElementsByClassName('product-rating').insertAdjacentHTML("beforeend", item[1]);
        document.getElementsByClassName('review-rating').removeChild();
        document.getElementsByClassName('review-rating').insertAdjacentHTML("beforeend", item[1]);
        document.getElementById('average-rating').innerHTML = item[0];
        document.getElementById('total-review').innerHTML = item[0];
    }

...
class Product extends Db
{
    ...
    {
        $sql = self::$connection->prepare("SELECT * FROM `products`, `protypes` WHERE `products`.type_id = `protypes`.type_id AND DATEDIFF(CURDATE(), DATE('2021-11-30')) < 30 ORDER BY products.`created_at`");
        $sql->execute();
        $items = array();
        $items = $sql->get_result()->fetch_all(MYSQLI_ASSOC);
        return $items;

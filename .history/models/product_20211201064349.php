...
class Product extends Db
{
    ...
    {
        $sql = self::$connection->prepare("SELECT `manufactures`.`manu_name` FROM `products` INNER JOIN `manufactures` ON `products`.`manu_id` = `manufactures`.`manu_id` AND `promotion_id` != 0 GROUP BY `products`.manu_id ");
        $sql->execute();
        $items = array();
        $items = $sql->get_result()->fetch_all(MYSQLI_ASSOC);
        return $items;
    ...
    {
        $sql = self::$connection->prepare("SELECT count(manu_id) AS dem FROM `products` WHERE `promotion_id` != 0 GROUP BY manu_id");
        $sql->execute();
        $items = array();
        $items = $sql->get_result()->fetch_all(MYSQLI_ASSOC);
        return $items;
    ...
    {
        $sql = self::$connection->prepare("SELECT * FROM `products`, `protypes` WHERE products.type_id = protypes.type_id and `type_id`=2");
        $sql->execute();
        $items = array();
        $items = $sql->get_result()->fetch_all(MYSQLI_ASSOC);
        return $items;

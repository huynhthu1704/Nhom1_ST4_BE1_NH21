...
class Product extends Db
{
    ...
    {
        $sql = self::$connection->prepare("SELECT * FROM `products`, `protypes`, discount WHERE products.type_id = protypes.type_id and products.`type_id`= 1 and products.discount_id = discount.dis_id");
        $sql->execute();
        $items = array();
        $items = $sql->get_result()->fetch_all(MYSQLI_ASSOC);
        return $items;

...
class Review extends Db {
    public function insertReview($name, $rv_content, $email, $rating, $product_id) {
        $sql = self::$connection->prepare("insert into reviews(review_id,name, email, rv_content, rating, product_id) values(NULL,?, ?, ?, ?, ?, CURENT_TIMESTAMP())");
    ...
    public function getNewReview() {
        $sql = self::$connection->prepare("SELECT * FROM reviews ORDER BY ");
        $sql->execute();
        $item = array();
        $item = $sql->get_result()->fetch_all(MYSQLI_ASSOC);
        return $item;

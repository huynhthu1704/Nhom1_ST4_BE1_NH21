...
class Product extends Db
{
    ...
    {
        $sql = self::$connection->prepare("SELECT * FROM `products`, protypes WHERE products.type_id = protypes.type_id and products.quantity > 0 and `name` LIKE ? AND products.`type_id` = ?");
        $keyword = "%$keyword%";

...
class Product extends Db
{
    ...
    {
        $sql = self::$connection->prepare("SELECT * FROM `products`, protypes WHERE `name` LIKE ?");
        $keyword = "%$keyword%";
    ...
    {
        $firstLink = ($page - 1) * $perPage;
        $sql = self::$connection->prepare("SELECT * FROM `products` WHERE `products`.`name` LIKE ? LIMIT ?, ?");
        $keyword = "%$keyword%";

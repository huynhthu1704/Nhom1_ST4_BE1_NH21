...
class Product extends Db
{
    ...
    {
        $sql = self::$connection->prepare("SELECT * FROM `products`, protypes, discount WHERE products.type_id = protypes.type_id and products.discount = discount.dis_id and `discount_id` != 0");
        $sql->execute();
        $items = array();
        $items = $sql->get_result()->fetch_all(MYSQLI_ASSOC);
        return $items;

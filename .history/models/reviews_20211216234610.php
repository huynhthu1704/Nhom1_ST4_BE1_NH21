...
class Review extends Db
{
    ...
    public function get3Reviews($id, $page, $perPage)
    {
        $firstLink = ($page - 1) * $perPage;
        $sql = self::$connection->prepare("SELECT * FROM reviews WHERE product_id = $id ORDER BY review_id desc LIMIT $firstLink, $perPage");
        $sql->execute();
        $item = array();
        $item = $sql->get_result()->fetch_all(MYSQLI_ASSOC);
        return $item;
    ...
    public function paginate($url, $page, $total, $perPage)
    {

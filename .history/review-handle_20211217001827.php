...
if ($rs != -1) {
    ...
    $totalLinks = ceil(count($review->get3Reviews((int)$product_id, $page, $perPage)) / $perPage);
    $link = "";
    for ($j = 1; $j <= $totalLinks; $j++) {
        if ($j == $page) {
            $link = "<li class=\"active\"><a onclick=\"paginate($product_id.','.$j.','.$perPage)\" href=\"\">$j</a></li>";
        } else {
            $link = "<li><a onclick=\"paginate($product_id.','.$j.','.$perPage)\" href=\"\">$j</a></li>";
        }
    }
    echo $link;
}

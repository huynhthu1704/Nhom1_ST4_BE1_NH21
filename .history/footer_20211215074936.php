...
<script>
	...
	function removeProductFrCart(id) {
		let cartId = "cart" + id;
		let cartElement = document.getElementById(cartId);
		cartElement.parentNode.removeChild(cartElement);
		let xmlhttp = new XMLHttpRequest();
		xmlhttp.onload = function() {
			document.getElementById('qty').innerHTML = this.responseText;
		}
		xmlhttp.open("GET", "cart-remove.php?id=" + id);
		xmlhttp.send();
	}
	...
	function decreaseQty(id) {
		...
		if (qty == 1) {
			...
			if (choice) {
				removeProductFrCart(id);
			}
		} else {
			document.getElementById(pID).value = qty - 1;
			$oldTotalQty = Number(document.getElementById('qty').innerHTML);
			document.getElementById('qty').innerHTML = $oldTotalQty- 1;
		}
	}
	...
	function increaseQty(id) {
		...
		xmlhttp.onload = function() {
			...
			if (qty >= qtyInStock) {
				...
				// document.getElementById(pID).value = qty + 1;
				// document.getElementById('qty').innerHTML = Number(document.getElementById('qty').innerHTML) + 1;
				// xmlhttp.onload = function() {}
				// xmlhttp.open("GET", "cart-add.php?id=" + id);
				// xmlhttp.send();
				addCart(id);
			}
		}
